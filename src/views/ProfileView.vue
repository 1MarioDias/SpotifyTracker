...
export default {
  ...
  computed: {
    ...
    userName() {
      // Show the lastfm username as the display name; fall back to username
      return this.user?.lastfm_username ?? this.user?.username ?? 'Guest';
    },
    email() {
        ...
        >
          <div class="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12 lg:gap-16 p-6 md:p-12 lg:p-16">
            ...
            <div class="flex-1 space-y-4 md:space-y-6 text-center md:text-left">
              <div>
                <h1 class="text-5xl md:text-6xl lg:text-7xl xl:text-[77px] font-normal leading-tight">
                  {{ userName }}
                </h1>
                <p class="text-base md:text-lg mt-2 opacity-90">
                  {{ email }}
                </p>
              </div>

              <button class="bg-[#100C14] text-white px-8 py-3 rounded-full text-lg md:text-xl hover:bg-opacity-80 transition-all">
                Add last.fm Account
              </button>
            </div>
            ...
            <div class="flex-shrink-0 flex flex-col items-center gap-6 relative">
              <div class="flex items-center gap-0">
                <Badge color="gold" text="#1" />
                <Badge color="red" text="#2" />
                <Badge color="teal" text="8k" />
              </div>
